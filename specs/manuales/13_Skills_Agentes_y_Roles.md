...
| `axiom-concision-discipline` | La política de concisión al redactar (respuestas a un humano, resúmenes de cierre, o el cuerpo de otra skill): qué ceremonia recortar (repetir la petición, intros/cierres protocolarios, narrar operaciones triviales) y qué nunca se recorta (evidencia, restricciones, errores, razonamiento relevante, detalle ante riesgo/incertidumbre/revisión humana). Adopta la filosofía de concisión de Caveman — comprimir estilo, no sustancia — sin instalar la herramienta; distinta de `axiom-terminal-output-efficient` (comprime SALIDA DE TERMINAL vía RTK) y de `axiom-terse-comms` (atajo de símbolos opt-in, solo agente-a-agente, catálogo seed aparte en `workspace-skills.ts`). |

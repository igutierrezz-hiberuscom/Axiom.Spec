...
| 2 | Configurar roles / adapters / MCPs / providers | ✅ | `axiom.config/` con topology, mcp-manifest (sdd/spec/axiom), toolchain-catalog, skills-index (backend/frontend/qa/sdd/spec), profiles, providers=cmm |
...
| 9 | MCPs disponibles y responden | ✅ | doctor deep: liveness real (initialize) de sdd-mcp-server + spec-mcp-broker = pass; broker unificado `axiom` expone 15 tools; `spec.incrementRead` devuelve el spec migrado real; `axiom.topologyRead` devuelve la topología real |

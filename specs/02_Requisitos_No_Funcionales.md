...
Cada capa nueva (adapters, agents, toolchain) debe tener un GATE verificable por `@axiom/doctor` (p. ej. GATE 0031 / check `TC-009`: los **8** adapter packages activos deben tener `src/generator.ts` + `dist/index.js` materializados, o falla). Los aliases legacy no cuentan como packages activos y LiteLLM fue retirado. No basta con documentación: el doctor debe poder comprobarlo en runtime.

Complementariamente, la suite e2e `apps/cli/tests/e2e/` (`ACC-030`) da una garantía de calidad end-to-end para el setup de workspace + MCP: crea in-process un workspace multi-repo completo y lanza el único `axiom mcp serve --kind axiom` como proceso hijo, probando `server/discover`, `tools/list`, una lectura y una transición preview/confirmación sobre stdio JSON-RPC usando exactamente el `command`/`args` que el setup genera en `.axiom/mcp.yml`. Es cobertura test-only (sin cambio de producto): verifica que la config MCP generada es efectivamente lanzable, no solo declarativamente correcta. `INC-20260708-adapters-depth` mantiene la suite e2e de adapters (`adapters.e2e.test.ts`) que materializa in-process los targets activos y verifica el set completo de ficheros potenciales declarado en `GENERATED_FILES_BY_TARGET` por target. Los aliases legacy de adapters se prueban mediante normalización y los outputs opcionales con semántica no-clobber se distinguen de los archivos escritos en una corrida.

...
**Estado verificado el 2026-07-30: la suite no está verde.** El requisito de cobertura runtime sigue vigente, pero no debe confundirse con una fotografía actual de ejecución. El cierre histórico de `INC-20260708-product-repo-self-bootstrap` e `INC-20260708-fix-longstanding-test-failures` registró una suite sin fallos en ese momento; la ejecución global independiente del review reportó 328/330 archivos y 3425/3427 tests, con fallos en `packages/doctor/tests/agents.test.ts` y `packages/mcp-tools/tests/capability-routing-roundtrip.test.ts`, fuera del alcance de este incremento. Después se añadió una prueba focalizada de `extractVersion()` y ambos fallos se reprodujeron aisladamente. `npm run doctor` y `npm run readiness:first-project` también fallan actualmente en `TC-011` por un `bundleHash` stale de `axiom-reviewer`. Nota de taxonomía reconciliada: el model-routing tiene **7 slots** canónicos (`increment, bug, plan, implementation, qa-e2e, review, archive`), no 10 — cualquier cifra "10 slots" en documentación previa es obsoleta.

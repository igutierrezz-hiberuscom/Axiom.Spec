...
### RF-AXM-003 Modelo de topología de repos y mutación estructural

El modelo de topología vigente usa `TopologyManifest.schemaVersion: 2` como único contrato. El manifest autoral vive únicamente en `<axiomRepo>/axiom.config/topology.yaml` y declara `axiomRepo`, `codeRepos`, `legacyRepos`, `roles`, `assignments` y `qaLane`. `axiom.yaml` en repos `code`/`legacy` solo declara identidad y el puntero `axiomRepo`; no es fuente del grafo. La autoridad, el schema, la forma y la semántica se validan fail-closed: no se aceptan schema 1, aliases `sddRepo`/`specRepo`/`roleCodeRepositories`, copias locales ni fallback silencioso. El scope de spec prioriza `<authority>/specs`, usa la raíz solo para una autoridad dedicada con `role: spec` sin ese child y, en otro caso, conserva el default `axiom.spec` sin reabrir una proyección local.

Los bindings locales usan su contrato independiente `schemaVersion: 2`: solo admiten IDs del manifest, paths absolutos canonicalizados y estados físicos `present-directory | missing | not-directory | remote-unmaterialized`. Los fallos de autoridad, YAML, schema, binding o I/O deben conservarse como `TopologyError` y propagarse a CLI, launcher y MCP; nunca se convierten en un mapa vacío.

`workspace setup`, `workspace adopt`, `repo add` y `role add` deben calcular primero un `StructuralMutationPlan` sin escribir. El apply adquiere locks ordenados, recupera journals previos, replantea y verifica bytes, tipo, `realpath`, timestamps e identidad física antes de publicar. `axiom.yaml` solo modifica el bloque `AXIOM:MANAGED`, preserva bytes humanos exteriores y cualquier fallo estructural devuelve exit no cero o `recovery-required`; los fallos derivados post-commit se separan como warnings con la estructura ya comprometida.

### RF-AXM-064 Estado y reparación del workspace

`workspace.json` debe cumplir un único schema cerrado con identidad de proyecto, adapters/providers ordenados, timestamps `createdAt`/`updatedAt` y extensiones declaradas. Solo `ENOENT` inicializa ausencia; corrupción, schema futuro, campos desconocidos, tipos inválidos o identidad divergente fallan sin overwrite. Las actualizaciones son lockeadas, atómicas e idempotentes byte a byte.

`WORKSPACE_STEP_CATALOG` es la fuente única de ownership y de la matriz setup↔comando granular. Los comandos de reparación prevalidan todos sus targets antes del primer writer, reportan `created | updated | unchanged | skipped | failed` y no habilitan adapters/providers ni modifican el registry o `workspace.json`; esas altas pertenecen a comandos `add/enable` explícitos.
...
El runtime debe poder generar configuración específica por adapter target mediante una función `generate<Target>Config` de firma común (`Promise<Result<GeneratorResult, AdapterGeneratorError>>`). El vocabulario público activo es de **8 targets** (`opencode`, `claude-code`, `github-copilot`, `vscode`, `cursor`, `codex`, `antigravity`, `visual-studio-2026`), todos con paquete dedicado. `copilot-vscode` no es alias, tipo ni entrada pública: `init` lo rechaza. Sólo `configure` puede migrar el literal histórico persistido en `init.json#profileTriple.adapterTarget` a `github-copilot` antes de installer o dispatcher. LiteLLM está retirado. El nivel `SupportLevel` de model-routing (`fallback-only` para la mayoría de targets) es un eje separado de "tiene generador dedicado".

### RF-AXM-017 Superficie de comandos operativos más amplia que la documentada

La superficie operativa se define por los comandos registrados en `apps/cli/src/index.ts` y por sus funciones `run*`, no por un conteo histórico de ficheros. Incluye, entre otras, las superficies de workspace multi-repo, workflow (`axiom-increment`, `axiom-bug`, `axiom-plan`, `axiom-role` y `axiom state`), adapters, providers, MCP, memoria, toolchain, bootstrap, adopción y launcher. Los comandos que todavía no tengan documentación propia deben citarse con el contrato verificable de código y tests; la mera existencia de un fichero no convierte una capacidad en contrato estable. La vía real del workflow usa los comandos CLI y `@axiom/workflow`; no debe documentarse el catálogo interno de intent commands no cableado como punto de entrada de usuario.
...
La topología vigente usa exclusivamente `schemaVersion: 2`: `axiomRepo` identifica el repositorio gestionado, `codeRepos` los repos de implementación y `legacyRepos` las fuentes de solo lectura; las referencias usan discriminadores `kind` y, para legacy, `mode: 'read-only-source'` más `legacyFunction`. No existe auto-map ni warning de una forma `sddRepo`/`specRepo`: schema 1 y cualquier shape legacy se rechazan fail-closed. La adopción CREA un `<project>.axiom` hermano nuevo y migra la spec/context legacy dentro (formato Axiom), dejando los repos legacy byte-for-byte intactos (registrados en `legacyRepos[]`). Ver [03_Modelo_Operativo_y_Datos.md](03_Modelo_Operativo_y_Datos.md).
...
### RF-AXM-049 Registro vigente de adapter targets (8 activos; migración interna de estado legacy)

El vocabulario activo `AdapterTarget`/`ADAPTER_TARGETS` contiene 8 ids: `opencode, claude-code, antigravity, visual-studio-2026, cursor, github-copilot, vscode, codex`. `DEFAULT_TARGET` sigue siendo `opencode`. `copilot-vscode` no es alias, tipo ni entrada pública; exclusivamente `configure` puede migrar el literal persistido de `init.json#profileTriple.adapterTarget` a `github-copilot` antes de installer o dispatcher. `litellm` está retirado. El vocabulario activo está alineado entre CLI, install-profiles, `axiom.yaml`, installer registry y support-matrix. El invariante de reconciliación vive en NFR-AXM-021.

### RF-AXM-050 Generadores de adapter dedicados para codex/antigravity/visual-studio-2026 (`INC-20260726-adapter-generators`)

`codex` y `antigravity` tienen generators dedicados single-file (`.codex/AGENTS.md` y `.antigravity/AGENTS.md`). `visual-studio-2026` es un target canónico distinto que delega y reutiliza el writer de `github-copilot` para la salida común `.github/copilot-instructions.md`; no genera `.vs/AXIOM.md`. El nivel `'fallback-only'` de `SUPPORT_MATRIX` sigue siendo un eje separado de la existencia de generator.
...
### RF-AXM-059 Receipts de fase emitidos por la transición gobernada (`INC-20260730-phase-receipts`, ACC-041)

`writePhaseReceipt` (`@axiom/workflow`) no queda restringido al comando manual `axiom phase receipt`: `runGovernedTransition` lo invoca desde el boundary común cuando el caller habilita `receipt`. Así CLI, launcher y `sdd.transitionApply` MCP conservan una única ruta de emisión y no dependen de wrappers `…Core` privados ni de lógica de receipt duplicada por superficie. El receipt usa el nombre real de la transición (`increment-verify`, `bug-archive`…), el mismo `command` de `workflows.yaml`, sin mapeos inventados al vocabulario `design/tasks/apply/...`.

Los previews (`--preview`/`--dry-run` y preview MCP/launcher) no emiten receipt porque no aplican una transición. En ejecución, un fallo al escribir el receipt es best-effort/no bloqueante: se comunica como aviso y no revierte ni altera el resultado ya decidido de la transición. Para archive, la ruta común selecciona la carpeta ya movida a `_archive/<id>/` como destino, evitando recrear una carpeta fantasma pre-archive.

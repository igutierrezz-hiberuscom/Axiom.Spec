...
**Deferred by explicit instruction for this increment** ("Do NOT edit
`Axiom.Spec/specs/00..08`" — this increment's brief). This is a
standalone, single-increment fix (not part of a batch), so — following the
same pattern already established for other standalone/batch increments in
this repo — canonical spec integration (likely
`06_Integraciones_y_Capacidades.md`'s scaffolding-engine description and/or
`03_Modelo_Operativo_y_Datos.md`'s catalog/lockfile shapes) is left to a
future consolidation pass (e.g. via the `axiom-spec-integrator` flow),
which can also decide whether to fold this into the existing
INC-20260713-ns2-adapter-generation / INC-20260710-architect-member-handoff
narrative rather than adding a new isolated section. No stable knowledge
was lost: this README documents the durable facts (which files, which
hashes, which wiring points) in full.

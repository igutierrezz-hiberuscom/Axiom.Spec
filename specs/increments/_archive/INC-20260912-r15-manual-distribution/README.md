...
Verificado el 2026-09-12 sobre el inventario de `apps/cli/src/commands/workspace-setup.ts`: el repositorio autoral de un proyecto recibía `specs/README.md`, los nueve documentos `00..08`, `context/TECHNICAL_CONTEXT.md`, `context/README.md`, las carpetas `specs/{increments,bugs,archive}` y `context/{architecture,integrations,operations,references}`, los YAML de `axiom.config`, `axiom.skills.lock` y `.axiom/mcp.yml`. No recibía `docs/**` ni `templates/`. `docs/generated-files.md` confirmaba que lo que llegaba por target eran ficheros de instrucciones y de estado, no manuales. Este estado inicial quedó resuelto por la implementación de este incremento.
...
Completado. Las decisiones están cerradas y enlazadas; la revisión independiente
N=1 no encontró blockers y los gates finales pasaron antes del archive gobernado
por Core.
...
- `packages/document-bootstrap/tests/manual-distribution.test.ts`: PASS (6
	tests; idempotencia, stale, preview y manifests inválidos).
- Suites de setup, adopt, upgrade y cobertura documental focales: PASS (41
	tests).
...
- `git diff --check`: PASS; solo mostró avisos CRLF de archivos ya modificados
	en el worktree.
- Suite global `npm test`: 3742/3764 tests PASS en 346/358 archivos; los 22
	fallos quedaron clasificados como contratos/timeouts preexistentes de R-14/R-13
	(incluida la repetición de `workspace-step-reconciliation.test.ts`), sin
	regresión nueva atribuible a R-15.
...
Implementación realizada; estado `archived` después de receipt verify, freeze,
review independiente, gates finales y archive gobernados por Core.

## General spec integration

La consolidación estable se integró una sola vez en `specs/00..08` y
`context/**`: manual runtime único en `Axiom/docs/**`, distribución bajo
`docs/axiom/`, fuente única de templates y gate documental estructurado.

...
Todas las decisiones D-01 a D-06 han sido formalizadas como artefactos `decision` mediante `axiom axiom-decision create` y vinculadas mediante `link-plan` y `link-increment`:
- `DEC-20260911-223522-6ox3vz`: D-01 Consolidación de self-update transaccional y shim de instalación.
- `DEC-20260911-223523-jii9h2`: D-02 Desversionado de dist de adapters y honestidad en TC-009.
- `DEC-20260911-223524-dfl08s`: D-03 Derivación determinista de versión de runtime desde metadata de build.
- `DEC-20260911-223525-y8j1xc`: D-04 Contrato de aviso y recomendación de modelo por adapter.
- `DEC-20260911-223526-127zvd`: D-05 Retirada física total de paquete TUI y saneamiento de dependencias.
- `DEC-20260911-223526-pxpi06`: D-06 Soporte de múltiples instancias en workflow-state con selección explícita.
...
Validación documental y de consistencia: 6 decisiones formalizadas, trazabilidad bidireccional hacia plan e incremento, validación de índice (`axiom index validate` OK con 28 artefactos).
...
Validado. Criterios AC-DEC-01 a AC-DEC-04 satisfechos:
- 6 decisiones formalizadas con justificación y trazabilidad completa.
- `axiom index validate` en verde (28 metadatas válidos).
- Sin cambios de código en `Axiom/`.

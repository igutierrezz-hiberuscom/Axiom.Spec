...
review quedaron resueltos; el freeze y los receipts finales se conservan junto
al incremento archivado.
...
- Después del build, `packages/project-resolution/dist/resolver.d.ts` expone
	...
	paquete.
- El freeze final se conserva en `candidate-freeze.json`, se valida con
			`checkCandidateFreeze` y su receipt tiene hash recomputable.
- `axiom freeze` y `axiom phase receipt` aceptan también incrementos ya
			archivados bajo `specs/increments/_archive/`.
...
entrada legacy. El incremento queda `closed` y archivado con

...
ACC-063, ACC-064 y ACC-069 quedaron implementadas: el workspace resuelve desde
la autoridad sin depender del catálogo, `--no-register` es operativo, se
retiraron `repo attach` y los kinds ambiguos, y `WorkspaceStateV1` carga
fail-closed con actualización lockeada/atómica/no-op estable. La revisión final
del orquestador contra criterios quedó en GO.

Evidencia dirigida: `15/15` suites y `186/186` tests, cinco typechecks y build;
la regresión final del lote volvió a pasar incremental `42/42`, setup/adopt
`25/25`, workspace command `19/19`, app/launcher `53/53`, doctor y readiness.
Los fallos transitorios documentados en el receipt antiguo no se reproducen en
la validación final y no se usan como evidencia de cierre.

## Integración estable

Integrada en `specs/00..08`, contexto de datos/onboarding y plan integral. La
spec activa distingue autoridad topológica, catálogo opcional y estado
project-scoped estricto; se retiraron claims de resolución obligatoria por
`projects.yml` y resets silenciosos de `workspace.json`.

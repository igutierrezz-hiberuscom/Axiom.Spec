...
Integrated into `Axiom.Spec/specs/00_Resumen_Ejecutivo.md` and
`Axiom.Spec/specs/05_Interfaces_Operativas.md`: the default open/redirect
behavior and removal of the superseded root PWA shell are now explicit.
Technical-context outcome: the launcher-default behavior and the current
CLI surface are reflected in
`Axiom.Spec/context/architecture/03-ciclo-de-vida-cli-y-orquestacion.md`
and `Axiom.Spec/context/TECHNICAL_CONTEXT.md`.

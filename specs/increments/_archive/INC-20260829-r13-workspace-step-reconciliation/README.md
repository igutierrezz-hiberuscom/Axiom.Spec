...
ACC-068 quedó implementada mediante `WORKSPACE_STEP_CATALOG` y su matriz derivada.
Adapters repara process surfaces sin invadir MCP; rules conserva regiones humanas
de `AGENTS.md`; config-scaffold cubre sus siete outputs; MCP prevalida target/path;
y los resultados por step usan el vocabulario tipado acordado. La revisión
independiente concluyó GO.

Evidencia: ocho suites/`124` tests, typecheck de CLI/document-bootstrap y
diff-check en PASS; la regresión final de YAML/MCP/steps/envelopes/bindings pasó
`32/32`, además de build, doctor y readiness del lote.

## Integración estable

Integrada en `specs/01`, `03`, `05`, `07`, `08` y en contexto de arquitectura y
onboarding como contrato único add/enable frente a repair/regenerate. No se creó
Workbench, MCP obligatorio, índice nuevo ni arquitectura paralela.

...
El punto de entrada guiado vigente es `axiom app`, que abre el launcher web local bajo `/launcher/`. Desde allí se puede instalar o unir un proyecto, configurar un workspace, adoptar spec/SDD/contexto, consultar el registry, operar el ciclo SDD con preview y confirmación, y gestionar las pantallas de operador de Model Routing y Actualización de Axiom (ACC-082). La misma capacidad está disponible de forma headless mediante los comandos CLI y sus handlers MCP. `axiom` sin subcomando ya no abre una TUI y `@axiom/tui` fue eliminado físicamente del árbol de paquetes y dependencias (ACC-081); `axiom init` sigue disponible como comando no-interactivo/scriptable single-repo. Los workflows de artefacto admiten múltiples instancias simultáneas en vuelo con selección explícita mediante `--id` y el subcomando `select` (ACC-087).
...
`runWorkspaceSetup`/`runWorkspaceAdopt` y comparte con la CLI el mismo
preflight de ownership y solapamientos. `setup`, `repo add` y `role add`
rechazan identidades foráneas o ambiguas antes de escribir; `adopt` solo puede
preservar como `skipped` una identidad foránea válida y explícita. Los recursos
estructurales se publican como una unidad recuperable; únicamente los outputs
derivados post-commit pueden degradarse a warnings sin revertir una estructura
válida.
...
En este workspace, `Axiom.Spec/` es el repositorio canónico de especificación. Cuando el workspace se declara mediante topología schema 2, la autoridad única es `Axiom.Spec/axiom.config/topology.yaml`, cuyo `axiomRepo` identifica ese repositorio; `Axiom/axiom.yaml` conserva identidad de repo `code` y un puntero local hacia la autoridad. No existe un `specRepo` autoral ni una copia de topology por repositorio. `Axiom/axiom.spec/` se conserva como baseline product-owned materializable del runtime dogfoodeado; sus contenidos no sustituyen el repositorio canónico ni deben fusionarse con él (ADR-0032).
...
- `Axiom.Spec/specs/decisions/` (las decisiones gestionadas, incluidas las nueve migradas desde la antigua raíz `decisions/`) y `Axiom.Spec/specs/adr/ADR-0032-toolchain-versioning/` (el ADR independiente de toolchain);
...
## Front visual de onboarding y manual runtime (2026-07-15 en adelante) — tanda INC-20260715-* + R-15

...
## Paridad de alcance de adapters + front de onboarding del launcher (2026-07-26) — tanda INC-20260726-*

Tanda de 7 incrementos (spec-first, gate verde por incremento) que lleva el conjunto de adapters a **paridad de primera clase** y convierte el launcher web (`axiom app`) en un front de onboarding config-rich, con probes de runtime opt-in en `doctor`:

- **Registro canónico de adapters (10 targets)**: el vocabulario `ADAPTER_TARGETS` se reconcilia a fuente única (10 ids) a través de CLI / `axiom.yaml` / install-profiles / support-matrix; `codex` entra como target nuevo y `vscode` pasa a ser target de primera clase (distinto de `copilot-vscode`, con generador propio).
- **Generadores dedicados**: `codex`/`antigravity`/`visual-studio-2026` ganan paquete `@axiom/adapters-<target>` real (merge-preserving), no el fallback delgado — 9 paquetes de adapter dedicados (solo `copilot-vscode` queda sin uno).
- **Paridad de MCP nativo**: `vscode` (`.vscode/mcp.json`) y `visual-studio-2026` (`.vs/mcp.json`, asunción documentada override-able) reciben MCP nativo real; `codex`/`antigravity` reciben una nota informativa honesta (su MCP es user-global, nunca un fichero inventado). La superficie portable `.axiom/{skills,agents}` es incondicional (todo adapter es descubrible por ella).
...
- **Doctor con probes de runtime (opt-in)**: `axiom doctor --deep` (y `?deep=1` en el launcher) añade un probe funcional de tool (`TC-018`) y un handshake `initialize` real de liveness de MCP (`TC-019`); aditivo y never-fail (nunca cambia el veredicto del doctor síncrono).
...
## Knowledge Harvest: memoria viva entre fases SDD (2026-07-29) — tanda INC-20260729-knowledge-*

Tanda de 3 incrementos que implementa el flujo de memoria viva entre fases del ciclo SDD usando Engram como backend, con metadata de fase trazable y un comando de harvest para clasificar y proponer promociones a contexto técnico y skills. La memoria compartida entre fases se logra a través de la superficie MCP de Engram (ya existente, `@axiom/memory` + `engram-backend.ts`), sin requerir sincronización Git de `.engram/` ni base de datos centralizada.

- **Metadata de fase en `MemoryEntry`** (`INC-20260729-knowledge-phase-metadata`): 7 campos opcionales aditivos (`increment`, `phase`, `actorRole`, `knowledgeKind`, `stability`, `visibility`, `sourceArtifact`) + 5 tipos cerrados (`SddPhase`, `ActorRole`, `KnowledgeKind`, `Stability`, `Visibility`) con round-trip completo en ambos backends (engram vía frontmatter YAML-like en `content`, in-memory vía JSON). Back-compat total: 55/55 tests en verde sin modificar.
- **Comando `axiom knowledge harvest`** (`INC-20260729-knowledge-harvest-command`): lee todas las memorias de un incremento, las clasifica por `stability` en 4 grupos (candidate-context, candidate-skill, historical-only, discarded) y genera `knowledge-harvest.md` en la carpeta del incremento. Read-only: nunca muta contexto técnico ni skills. `--dry-run` imprime el reporte sin escribir.
- **Comandos `axiom knowledge sync` y `axiom knowledge pull`** (`INC-20260729-knowledge-sync-command`): `sync` exporta memorias del incremento a `.engram/chunks/` (JSON versionable, append-only) + `manifest.json` y hace git add/commit/push en `<project>.axiom`. `pull` hace git pull e importa chunks nuevos a la BD local de Engram vía MCP. `.engram/engram.db` está gitignored. Incluye validación anti-secretos y filtro de `visibility: 'private'`.
- **Contrato de memoria en skills de fase** (`INC-20260729-knowledge-skill-contract`): el manual `13_Skills_Agentes_y_Roles.md` incorpora una sección "Contrato de memoria Engram por fase" con metadata obligatoria, reglas generales (qué guardar, qué NO guardar), reglas específicas por fase (analysis, architecture, frontend, backend, QA/validator), flujo de transferencia entre fases, y relación con el Knowledge Harvest.

### Decisiones de arquitectura

1. **Sync vía `.engram/` chunks + Git**: Engram (`internal/sync`) exporta memorias como chunks JSON comprimidos append-only en `.engram/chunks/` + `manifest.json` mergeable. `axiom knowledge sync` escribe chunks versionables y hace git add/commit/push en `<project>.axiom`. `axiom knowledge pull` hace git pull e importa chunks nuevos. `.engram/engram.db` está gitignored. Sin base de datos centralizada.
2. **Promoción diferida**: `axiom knowledge harvest` solo PROPONE promociones a contexto técnico y skills. La promoción real (`axiom knowledge promote`) requiere revisión humana o confirmación explícita y queda como follow-up.
3. **Metadata codificada en contenido**: Engram no soporta campos estructurados nativos. La metadata de fase se codifica como frontmatter YAML-like (`---\n...\n---\n`) al inicio del `content`, decodificado en lectura vía `mem_get_observation` (contenido completo, sin truncar a 300 chars).
4. **Validación anti-secretos**: `axiom knowledge sync` escanea cada memoria contra patrones de secretos/tokens/credenciales antes de exportar. Las memorias con `visibility: 'private'` nunca se exportan.

...
2. adapters de ejecución (**8** packages `@axiom/adapters-*` dedicados desde `INC-20260726-adapter-generators`; LiteLLM fue retirado y `copilot-vscode` no es un target público: solo `axiom configure` migra ese literal cuando ya está persistido en `init.json`, antes de instalar o despachar; ver "Paridad de alcance de adapters + launcher onboarding" al final);
3. surfaces basadas en MCP (catálogo vía `@axiom/toolchain`, comandos `axiom mcp`, `axiom toolchain` y el server ejecutable `@axiom/mcp-server`); el único kind admitido es `axiom`, lanzado con `axiom mcp serve --kind axiom`, y el único broker gestionado emitido es `axiom-mcp-broker`, siempre fijado al proyecto al que sirve;
...
| `github-copilot`, `vscode`, `cursor`, `codex`, `antigravity`, `visual-studio-2026` | `fallback-only` | No |
...
**Ojo — dos ejes SEPARADOS (actualizado por `ACC-025..ACC-029`)**: el `SupportLevel` de esta tabla mide SOLO la proyección de model-routing per-slot. NO debe confundirse con "tiene generador dedicado / MCP nativo": `SUPPORT_MATRIX` tiene 8 entradas activas, los **8 targets** tienen paquete `@axiom/adapters-<target>` dedicado y **5** reciben config MCP nativa de proyecto. `copilot-vscode` no participa en esos conjuntos públicos; solo `configure` puede migrar ese literal cuando está persistido en `init.json`. LiteLLM fue retirado. Un target puede ser `fallback-only` en routing y a la vez tener generador dedicado + MCP nativo real.

### Contrato vigente de instrucciones Copilot (ACC-024)

`github-copilot` escribe la instrucción general en
`.github/copilot-instructions.md`, la ubicación oficial de instrucciones de
repositorio para GitHub Copilot y VS Code. `.vscode/` queda para
`settings.json`, `extensions.json` y `mcp.json`; no se usa
`.vscode/copilot-instructions.md` como salida activa. Las process surfaces por
ruta se escriben una sola vez en `.github/instructions/<id>.instructions.md`.
...
- **`createStdioMcpClient({ command, args, cwd, env })`**: cliente MCP stdio LOCAL reusable — spawnea un proceso hijo local, habla JSON-RPC 2.0 delimitado por saltos de línea (`server/discover`/`tools/list`/`tools/call`) con metadata de protocolo MCP `2026-07-28`, timeouts y shutdown limpio. Es el mismo framing hecho a mano de `@axiom/mcp-server` (sin `@modelcontextprotocol/sdk`, sin dependencias npm nuevas), pero es un CLIENTE (escribe requests al stdin del hijo, lee del stdout) — módulo separado del server transport.
...
### Backend obligatorio de memoria Engram — R-12

`@axiom/memory` usa exclusivamente Engram local como `MemoryBackend` cross-session. `createEngramBackend` inicia `engram mcp --project=<projectId> --tools=agent` por stdio, hace el handshake estándar `initialize`, conserva el aislamiento GATE 0024 y mapea `save`/`load`/`query`/`saveSessionSummary` a `mem_save`/`mem_search`/`mem_get_observation`/`mem_session_summary`. El proceso aporta el scope; las tools no reciben un `project` redundante y el cliente rechaza cualquier `projectId` cruzado.

`resolveMemoryBackend` no selecciona ni degrada a JSON: un ejecutable ausente, spawn, handshake o tool fallida devuelve `MemoryError` tipado y visible al caller. No existen `createInMemoryBackend`, `memoryFilePath`, `forceJson` ni otros fallbacks de persistencia. Los JSON históricos permanecen sin leer, migrar ni borrar. TC-024 usa únicamente `engram --version` (nunca arranca el servidor stdio) y devuelve `fail` con instrucciones de instalar o reparar Engram cuando no está disponible.
...
> Este bloque documenta la primera entrega de 17 capability ids y no representa el conjunto completo actual. En aquella evolución coexistieron brokers `sdd`, `spec`, `memory` y `axiom`; el runtime vigente conserva 25 capability ids pero los sirve únicamente mediante `McpServerKind = 'axiom'` y `axiom-mcp-broker`.
...
### Server MCP ejecutable (`@axiom/mcp-server` + `axiom mcp serve`) — contrato vigente unificado

El paquete `@axiom/mcp-server` implementa el único server MCP gestionado vigente, `axiom-mcp-broker`. Habla JSON-RPC 2.0 sobre stdio delimitado por saltos de línea y expone los handlers registrados de `@axiom/mcp-tools`. Se lanza exclusivamente con `axiom mcp serve --kind axiom --project-root <path> [--home-dir <path>]` (ver [05_Interfaces_Operativas.md](05_Interfaces_Operativas.md)); `sdd`, `spec` y `memory` ya no son valores de `McpServerKind`.

- **Protocolo**: `createMcpServer` es un dispatcher JSON-RPC 2.0 puro y transport-agnostic para MCP `2026-07-28`. Atiende `server/discover`, `tools/list` y `tools/call` cuando la request incluye la metadata de versión soportada; `notifications/cancelled` cancela una llamada pendiente. `initialize` y `notifications/initialized` se rechazan como métodos legacy (`-32601` cuando corresponde), igual que cualquier método desconocido. `runStdioServer` es el bucle de transporte stdio (líneas JSON delimitadas por `\n`, tolerante a líneas en blanco, `-32700` en errores de parseo con id recuperable).
- **Conjunto de herramientas**: el kind único `axiom` expone el catálogo registrado de capabilities `sdd.*`, `spec.*`, `memory.*` y `axiom.*` bajo el mismo binding project-scoped. El registro global contiene 25 capability ids y los input builders mantienen los límites de cada operación.
...
- **Config de lanzamiento en `mcp.yml`**: `runWorkspaceSetup` emite una sola entrada `axiom-mcp-broker` con `command` y `args` para `axiom mcp serve --kind axiom --project-root <path absoluto>`. Los dos IDs previos (`sdd-mcp-server`, `spec-mcp-broker`) se conservan en el writer nativo solo como IDs de limpieza de configuración stale; no se emiten como desired entries.
- **Config MCP nativa por herramienta**: la ruta de setup de workspace emite, por cada repo × adapter seleccionado, el schema nativo real que cada herramienta lee para auto-lanzar `axiom-mcp-broker`. Ya no se escribe el antiguo formato custom-shape (`{schemaVersion, projectId, servers:[...]}`) de `.opencode/mcp.json`/`.claude/mcp.json`; `.axiom/mcp.yml` permanece como fuente canónica.
- **Sin dependencias npm nuevas**: el framing JSON-RPC/stdio es hecho a mano (no `@modelcontextprotocol/sdk`), minimalista pero suficientemente correcto para que el cliente local haga `server/discover` + `tools/list` + `tools/call` con metadata MCP `2026-07-28`.
- **Aislamiento project-scoped enforced**: el server queda atado a UN proyecto al arrancar. Todo campo que identifica proyecto en los argumentos de `tools/call` (`projectId`/`homeDir`/`projectRoot`/`specScopeAbsolutePath`/`sddScopeAbsolutePath`) se fija al `context`; un valor que apunte a otro proyecto se rechaza con `isError: true`. Los selectores dentro del proyecto (`id`, `planId`, `skillId`, `specRelPath`, `role`, `taskTags`, `targetRepoId`, …) siguen siendo caller-supplied. Detalle completo en [07_Gobierno_y_Seguridad.md](07_Gobierno_y_Seguridad.md).

### Config MCP nativa por herramienta destino — INC-20260708-mcp-native-config-mapping

> **Registro histórico de INC-20260708:** la tabla de abajo conserva el despacho por target de aquella entrega. El contrato vigente difiere tanto en los targets (`visual-studio-2026` no recibe fichero y LiteLLM está retirado) como en el broker: `runWorkspaceSetup` proyecta una única entrada `axiom-mcp-broker`.

La entrega original resolvió el mapeo de las entonces dos entradas `sdd-mcp-server`/`spec-mcp-broker` al schema nativo. Hoy el mismo writer recibe una única entrada ejecutable `{ id: 'axiom-mcp-broker', command, args, env? }`; los IDs anteriores no se emiten y solo se reconocen para limpiar configuración stale.
...
- **Tabla fichero + schema nativo por target** (despacho INC-20260708; ver nota de actualización arriba):
  ...
  | `github-copilot`, `vscode` | `.vscode/mcp.json` | `{ "servers": { "<id>": { type:'stdio', command, args, env? } } }` (clave `servers`, `type:'stdio'` obligatorio) |
...
- **`.axiom/mcp.yml` sigue canónico**: `writeWorkspaceMcpConfig` solo escribe `.axiom/mcp.yml` con la entrada única `axiom-mcp-broker`; ya no llama a `generateOpencodeMcpJson`/`generateClaudeCodeMcpJson`. Los generadores custom-shape antiguos permanecen exportados y cubiertos por tests de proyección, pero no tienen call site en workspace-setup. La cobertura vigente del broker unificado vive en `packages/mcp-server/tests/axiom-broker.e2e.test.ts` y `server.test.ts`: usa `kind=axiom`, `server/discover`, `tools/list` y `tools/call` bajo un único binding project-scoped. La antigua `apps/cli/tests/e2e/workspace-mcp.e2e.test.ts` de procesos `sdd`/`spec` está íntegramente comentada y se conserva solo como historia.
...
- **`.axiom/mcp.yml`** (`McpProjectConfig`, `schemaVersion: 1`) se escribe en el repo de control, con exactamente dos entradas `McpServerEntry`, ambas `enabled: true`, `scope: 'repo'`:
  ...
  En aquella versión, ambas entradas llevaban un comando de lanzamiento real (`command: 'axiom'` + `args: ['mcp','serve','--kind',<sdd|spec>,'--project-root',<path absoluto>]`). Los `targetRepo` referenciaban claves del mapa de repos del registro v2. Este snapshot es exclusivamente histórico: la forma vigente emite una sola entrada `axiom-mcp-broker` con `--kind axiom`; los dos IDs descritos aquí ya no son desired entries.
...
> La tabla siguiente es una fotografía histórica de la primera entrega. El despacho vigente (actualizado por `ACC-025..ACC-029` y R-10) cubre **8 targets activos** y **8 paquetes dedicados**; `litellm` fue retirado y `copilot-vscode` no es un target público, sino un literal que `configure` puede migrar únicamente si ya está persistido en `init.json`. Prevalece la sección "Paridad de alcance de adapters + launcher onboarding" y el detalle canónico en `Axiom.Spec/context/architecture/04-adapters-y-model-routing.md`.
...
- **Superficie en `axiom context status`**: un bloque `delegationSuggestions` best-effort que lee un `session-metrics.json` OPCIONAL en `.axiom-state/<projectKey>/`; si está ausente, malformado, o todas las métricas están debajo de umbral, el bloque se omite silenciosamente (nunca falla `axiom context status`).
...
- **Code-intel MCP auto-cableado en repos de código cuando está habilitado** (`INC-20260713-ns3-codeintel-mcp`, actualizado por `INC-20260724-*` y R-10): las configs MCP nativas de los repos de CÓDIGO (`.mcp.json`/`.cursor/mcp.json`/`.vscode/mcp.json`/`opencode.json`) reciben una entrada por provider code-intel HABILITADO, `cmm` o `serena`, apuntada a ese repo, junto a `axiom-mcp-broker` cuando el binding del proyecto está confirmado. Fuente única `buildCodeIntelNativeServers` (`@axiom/providers`). Default vacío → nada emitido; el repo de SPEC no recibe code-intel nativo.
...
El modelo vigente usa **un solo** broker `axiom` (`axiom-mcp-broker`) y un único `McpServerKind = 'axiom'`. El broker expone el catálogo registrado completo: capabilities `spec.*`, las operaciones `sdd.*`, las capabilities `memory.*` y las lecturas `axiom.topologyRead`, `axiom.migrationManifestRead` y `axiom.adoptionStateRead`, todo bajo el mismo binding project-scoped. El registro contiene 25 capability ids. Los brokers separados `sdd`/`spec` y el kind `memory` son historia de la evolución y no siguen disponibles como rutas de CLI; Engram continúa como proceso MCP local de terceros, proyectado aparte en la configuración nativa cuando está habilitado.
...
> **Actualización (ACC-025..ACC-029, 2026-08-10; reconciliada por R-10):** el vocabulario canónico vigente es de **8 targets** (`opencode, claude-code, github-copilot, vscode, cursor, antigravity, visual-studio-2026, codex`). `litellm` fue retirado del contrato activo y no queda ningún export ni registro activo que lo represente. `copilot-vscode` no es un alias público: `axiom configure` solo puede migrar ese literal si ya estaba persistido en `init.json`, antes de instalar o despachar. El detalle canónico completo vive en `Axiom.Spec/context/architecture/04-adapters-y-model-routing.md`; esta subsección conserva la fotografía de la entrega INC-20260726 como historial.
...
> **Actualización (ACC-027, 2026-08-10):** `visual-studio-2026` **no genera `.vs/AXIOM.md`** como salida activa. Se proyecta sobre la instrucción común `.github/copilot-instructions.md` (mismo writer que `github-copilot`) y conserva su package como wrapper de compatibilidad para su propia ruta de generación. El detalle canónico vive en `Axiom.Spec/context/architecture/04-adapters-y-model-routing.md`.

### Paridad de MCP nativo + superficie portable incondicional — INC-20260726-adapter-mcp-parity, actualizado por ACC-025..ACC-029

`writeNativeMcpConfig` (`native-mcp-config.ts`) mantiene el contrato verificable: da a cada target un fichero MCP nativo real cuando hay schema verificado o una nota informativa honesta cuando no lo hay; nunca inventa un schema. Tabla de despacho vigente (ACC-029, reconciliada por R-10):

| Target | Fichero | Shape | Verificación |
|---|---|---|---|
| `claude-code` | `.mcp.json` | `{ mcpServers }` | VERIFIED |
| `cursor` | `.cursor/mcp.json` | `{ mcpServers }` | VERIFIED |
| `github-copilot` / `vscode` | `.vscode/mcp.json` | `{ servers, type:'stdio' }` | VERIFIED |
...
- `NATIVE_MCP_TARGETS` (gate de fichero de proyecto) = las primeras 4 filas (**5 ids**: `claude-code`, `cursor`, `github-copilot`, `vscode`, `opencode`). `NATIVE_MCP_INFORMATIVE_TARGETS` = `['codex','antigravity']` (segundo gate distinto, "tiene `case` explícito, sin fichero"). `litellm` fue retirado y no forma parte del contrato; `copilot-vscode` tampoco es un target MCP, pues solo `configure` puede migrar el literal persistido antes de todo dispatcher.
- La nota de `codex`/`antigravity` (`buildUserGlobalMcpNote`) nombra la ubicación user-global real y la clave esperada, pero no recomienda copiar ninguna entrada: sin un binding seguro al proyecto actual, Axiom entrega cero servers y un warning accionable.
...
- **`TC-019-axiom-mcp-broker`** (`runMcpServerLivenessCheck`): descubrimiento `server/discover` JSON-RPC MCP **real** contra el único server gestionado `axiom-mcp-broker`, usando el `command`/`args` leído de `.axiom/mcp.yml` (`loadMcpProjectConfig`) y `createStdioMcpClient` de `@axiom/providers`. Un proyecto sin `.axiom/mcp.yml` da `warn` honesto, no una adivinanza.

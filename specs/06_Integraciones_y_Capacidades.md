...
- **`resolveMemoryBackend(projectRoot, scope, opts?)`**: probe (un `server/discover` real, no solo un chequeo de PATH) y selecciona engram-si-disponible o el backend JSON de fallback. Nunca lanza; siempre devuelve un `note` de una línea.
...
- **Superficie MCP real en aquella entrega**: `memory.decisionRecall`/`memory.contextRecall` ganaron handlers en `@axiom/mcp-tools` y se expusieron entonces mediante `axiom mcp serve --kind memory`. El runtime vigente conserva esas capabilities dentro del broker único `axiom-mcp-broker`; `memory` ya no es un `McpServerKind` aceptado.

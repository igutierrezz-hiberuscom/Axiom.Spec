...
2. adapters de ejecución (**8** packages `@axiom/adapters-*` activos desde `INC-20260726-adapter-generators`; ver "Paridad de alcance de adapters + launcher onboarding" al final);
3. surfaces basadas en MCP (catálogo vía `@axiom/toolchain`, comandos `axiom mcp`, `axiom toolchain` y el único server ejecutable `axiom-mcp-broker` de `@axiom/mcp-server`); se lanza con `axiom mcp serve --kind axiom`, exige un binding project-scoped único y expone conjuntamente los dominios internos `sdd`, `spec`, `memory` y `axiom`;
...
| `antigravity`, `visual-studio-2026` | `fallback-only` | No |

Esto reemplaza la lista aspiracional previa de "targets iniciales" (Opencode CLI principal, Antigravity IDE secundario, VS Code+Copilot IDE principal, Claude Code CLI secundario): en el código actual, `opencode` es el único target con soporte de routing per-slot profundo. `copilot-vscode` se conserva solo como alias legacy de `github-copilot` y LiteLLM ya no es un target activo.

**Ojo — dos ejes SEPARADOS (actualizado por `ACC-025..ACC-029`)**: el `SupportLevel` de esta tabla mide SOLO la proyección de model-routing per-slot. No debe confundirse con "tiene generador dedicado / MCP nativo": los 8 targets activos tienen generator dedicado, pero solo `claude-code`, `cursor`, `github-copilot`, `opencode` y `vscode` reciben config MCP project-scoped verificada. Visual Studio no tiene schema MCP verificado; Codex y Antigravity usan configuración user-global y quedan cerrados por defecto. Un target puede ser `fallback-only` en routing y a la vez tener generador dedicado.

### Contrato vigente de instrucciones Copilot (ACC-024)

GitHub Copilot, Copilot CLI, VS Code y Visual Studio usan una única instrucción
general en `.github/copilot-instructions.md`. `copilot-vscode` solo se acepta
como alias legacy de `github-copilot`; `vscode` escribe únicamente
`.vscode/settings.json`, `.vscode/extensions.json` y `.vscode/mcp.json`. Las
process surfaces específicas se escriben una sola vez en
`.github/instructions/<id>.instructions.md`.

`configure`, `sync`, `workspace setup` y los adapters de Copilot/Visual Studio
convergen en el writer de `@axiom/document-bootstrap`. El resolver usa la plantilla versionada del
...
- **`createStdioMcpClient({ command, args, cwd, env })`**: cliente MCP stdio LOCAL reusable — spawnea un proceso hijo local, habla MCP `2026-07-28` delimitado por saltos de línea (`server/discover`/`tools/list`/`tools/call`), con metadata namespaced, timeouts y shutdown limpio. Es el mismo framing hecho a mano de `@axiom/mcp-server` (sin `@modelcontextprotocol/sdk`, sin dependencias npm nuevas), pero es un CLIENTE (escribe requests al stdin del hijo, lee del stdout) — módulo separado del server transport.
...
- **`resolveMemoryBackend(projectRoot, scope, opts?)`**: probe de `server/discover` MCP moderno, no solo un chequeo de PATH, y selección de engram-si-disponible o backend JSON de fallback. Nunca lanza; siempre devuelve un `note` de una línea.
...
- **Superficie MCP real**: `memory.decisionRecall`/`memory.contextRecall` tienen handlers reales en `@axiom/mcp-tools` y se exponen dentro del único `McpServerKind` `axiom` (`axiom mcp serve --kind axiom`). Engram sigue siendo backend local opcional; no existe un kind `memory` gestionado independiente.
- **GATE 0024 preservado y extendido**: la garantía de aislamiento cross-project se conserva y gana un segundo pin a nivel de proceso (`--project=`); el input-builder `memory.*` fija SIEMPRE `projectRoot` al `context` del server (nunca lee un `projectRoot`/`projectId` del llamador). Sin dependencia SQLite npm nativa — la SQLite local de Engram se usa exclusivamente como backend externo vía MCP stdio.
...
- **Merge-preserving y fuera de `.axiom/mcp.yml`**: reusa el merge por-id de los writers nativos (preserva servers/keys del usuario, escritura atómica). La entrada `engram` NUNCA se escribe en `.axiom/mcp.yml` — ese fichero es el config canónico del único broker `axiom-mcp-broker`; Engram es un server MCP local de terceros usado como backend de memoria, no un broker Axiom. La escritura nativa se dispara si hay broker axiom **o** si Engram está habilitado.
...
> Este bloque documenta la fotografía histórica de la primera entrega de 17 capability ids y no representa el conjunto actual. En aquella implementación, la separación de brokers `sdd`, `spec` y `memory` se conservaba por compatibilidad; ACC-030 sustituyó esa forma por el broker vigente único `axiom-mcp-broker`.
...
### Server MCP ejecutable (`@axiom/mcp-server` + `axiom mcp serve`) — ACC-030

El paquete `@axiom/mcp-server` implementa el único broker MCP real gestionado por Axiom: `axiom-mcp-broker`. Habla MCP `2026-07-28` sobre stdio delimitado por saltos de línea y expone todos los handlers de `@axiom/mcp-tools`. Se lanza con `axiom mcp serve --kind axiom --project-root <path> [--home-dir <path>]` (ver [05_Interfaces_Operativas.md](05_Interfaces_Operativas.md)).

- **Protocolo**: `createMcpServer` es un dispatcher JSON-RPC 2.0 puro y transport-agnostic que atiende `server/discover`, `tools/list`, `tools/call` y `notifications/cancelled`. Cada petición lleva metadata MCP namespaced; las respuestas incluyen `resultType`, identidad namespaced, `ttlMs`/`cacheScope` cuando corresponde y `-32022` para versiones no soportadas. `initialize`, `notifications/initialized` y `ping` devuelven `-32601`. `runStdioServer` es el bucle de transporte stdio de líneas JSON delimitadas por `\n`.
- **Superficie única**: `axiom` expone todas las capabilities `sdd.*`, `spec.*`, `memory.*` y `axiom.*`, incluidas lecturas y mutaciones confirmadas. El registro global contiene 25 capability ids; los dominios no representan procesos separados.
...
- **Config de lanzamiento en `mcp.yml`**: `runWorkspaceSetup`, `workspace-incremental`, `member-install` y worktrees materializan una única entrada con `command`/`args` para `axiom mcp serve --kind axiom --project-root <repo Axiom/control>`. El binding exige `type: axiom`, `scope: repo`, `targetRepo` registrado y coincidencia con el root del proceso.
...
- **Sin dependencias npm nuevas**: el framing JSON-RPC/stdio es hecho a mano (no `@modelcontextprotocol/sdk`) y cubre el contrato MCP `2026-07-28` de discovery/list/call sin handshake legacy.
- **Aislamiento project-scoped enforced**: el único broker está atado a UN proyecto al arrancar y exige un binding válido; cualquier `projectId`/`homeDir`/`projectRoot`/`specScopeAbsolutePath`/`sddScopeAbsolutePath` ajeno se rechaza, y los paths internos se validan contra las raíces reales del proyecto, incluidos escapes mediante symlink. `sdd.projectRegistryRead` devuelve únicamente la entrada del proyecto propio. Detalle completo en [07_Gobierno_y_Seguridad.md](07_Gobierno_y_Seguridad.md).

### Config MCP nativa por herramienta destino — INC-20260708-mcp-native-config-mapping

**Supersede el "caveat honesto (pendiente)"** de la sección anterior: mapear las entradas MCP de Axiom al schema nativo ya está entregado, pero la proyección queda condicionada a un binding project-scoped válido. `runWorkspaceSetup` valida proyecto, `mcp.yml`, manifest, `enabled` y `targetRepo` antes de escribir; la lógica vive en `native-mcp-config.ts` y `workspace-mcp.ts`.
...
- **Tabla fichero + schema nativo por target**:
  ...
  | `visual-studio-2026` | — (ninguno) | no hay schema verificado; se emite warning y no se escribe fichero |
...
- **Merge-preserving**: cada writer parsea el fichero nativo preexistente si lo hay, reemplaza o elimina **solo** las entradas Axiom-managed (keyed por id) dentro del mapa de servers de esa herramienta, y **preserva verbatim** cualquier otra key/server ya presente. Crea un fichero mínimo válido cuando el nativo está ausente y hay servers permitidos. Escritura **atómica** (tmp+rename). Un fichero nativo preexistente con JSON **malformado** se trata como best-effort-failed: warning + fichero dejado intacto.
- **Por repo × por adapter, en TODO el workspace**: `writeWorkspaceNativeMcpConfigs` recorre cada repo × adapter seleccionado después del filtro project-bound y acumula `filesCreated`/`warnings`. Los targets user-global no reciben escritura ni recomendación automática; el provisioning de worktrees usa el mismo gate.
- **`.axiom/mcp.yml` sigue canónico y sin cambios**: `writeWorkspaceMcpConfig` ahora **solo** escribe `.axiom/mcp.yml` (fuente canónica de Axiom, `McpProjectConfig` con `command`/`args` de lanzamiento reales, ver [03_Modelo_Operativo_y_Datos.md](03_Modelo_Operativo_y_Datos.md)); ya **no** llama a `generateOpencodeMcpJson`/`generateClaudeCodeMcpJson`. Los generadores custom-shape antiguos permanecen exportados en sus paquetes y cubiertos por sus tests, pero la ruta activa materializa solo `axiom-mcp-broker`. El E2E (`workspace-mcp.e2e.test.ts`) lanza el broker único leyendo los args del `.mcp.json` nativo y prueba discovery/list/call sobre stdio.
...
- **`.axiom/mcp.yml`** (`McpProjectConfig`, `schemaVersion: 1`) se escribe en el repo de control, con exactamente dos entradas `McpServerEntry`, ambas `enabled: true`, `scope: 'repo'`:
  ...
  En aquella versión, ambas entradas llevaban un comando de lanzamiento real (`command: 'axiom'` + `args: ['mcp','serve','--kind',<sdd|spec>,'--project-root',<path absoluto>]`). Los `targetRepo` referencian claves del mapa de repos del registro v2 (no paths). La forma vigente se describe en la sección ACC-030: una sola entrada `axiom-mcp-broker` y un único kind `axiom`. El caso degenerado de control y spec queda resuelto por el repo Axiom/control único.
...
> La tabla siguiente es la fotografía de la primera entrega y conserva el dato histórico de ocho targets. El despacho vigente cubre 10 targets, nueve paquetes dedicados y el único fallback compartido de `copilot-vscode`; prevalece la sección "Paridad de alcance de adapters + launcher onboarding".
...
- **Convergencia de la ruta de spec-scope (role-derived)** (`INC-20260713-fix-spec-scope-path`): CLI de ciclo de vida, broker MCP único y migración/adopción resuelven los artefactos (`increments/`/`bugs/`/`plans/`/`adr/`/`decisions/`) en el MISMO árbol. En un repo de spec DEDICADO (`role === 'spec'`) el rel-path es `.` (artefactos en la raíz del scope, sin `axiom.spec`); el resto mantiene `axiom.spec`. Fuente única: `resolveSpecRelPathForScope(role)` (`@axiom/workflow`).
...
**SUPERSEDE** el modelo de 2 brokers (`sdd` control + `spec` conocimiento) como contrato vigente para un repo `<project>.axiom`. Un code repo enlazado a `<project>.axiom` usa **un solo** broker `axiom` (`axiom-mcp-broker`, el único `McpServerKind`) que expone la unión completa de los dominios internos `sdd.*`, `spec.*`, `memory.*` y `axiom.*`, incluyendo todas las mutaciones SDD existentes bajo preview/confirmación. El registro contiene 25 capability ids y `sdd.gitRoleBranch` también pertenece a la superficie única. Engram/memory puede seguir usando Engram como backend/provider local, pero no existe un proceso `memory` gestionado separado. Los nombres de los brokers anteriores solo se conservan en artefactos históricos archivados.
...
### Registro histórico superado: 10 adapter targets — INC-20260726-adapter-registry-canonical

> HISTÓRICO: las subsecciones de esta tanda conservan la evolución original,
> pero no describen el contrato activo después de ACC-025..ACC-029. Para el
> estado vigente prevalecen las secciones superiores de targets soportados,
> instrucciones comunes y MCP project-bound.
...
### Registro histórico superado: generadores dedicados — INC-20260726-adapter-generators
...
### Registro histórico superado: paridad de MCP nativo — INC-20260726-adapter-mcp-parity
...
- La nota de `codex`/`antigravity` (helper `buildUserGlobalMcpNote`) nombra la ubicación user-global real, la clave esperada y el broker Axiom lanzable (`axiom-mcp-broker`), solo después de confirmar el binding del proyecto.
...
- **`TC-019-<serverId>`** (`runMcpServerLivenessCheck`): descubrimiento `server/discover` MCP `2026-07-28` **real** contra `axiom-mcp-broker`, usando el `command`/`args` leído de `.axiom/mcp.yml` (`loadMcpProjectConfig` de `@axiom/user-workspace`) y `createStdioMcpClient` de `@axiom/providers`. Un proyecto sin `.axiom/mcp.yml` (self-hosted) da `warn` honesto, no una adivinanza.

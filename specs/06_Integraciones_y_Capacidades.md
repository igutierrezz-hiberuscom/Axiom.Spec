...
- **Tabla fichero + schema nativo por target**:
  ...
  | `visual-studio-2026` | `.vs/mcp.json` | `{ "servers": { "<id>": { type:'stdio', command, args, env? } } }` (asunción documentada y override-able) |
  ...
  | `litellm` | — (ninguno) | warning genérico: no se inventa un schema MCP |

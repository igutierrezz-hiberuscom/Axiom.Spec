...
- **Onboarding visual** (`INC-20260715-launcher-onboarding`): nueva pestaña "Instalar / Unirse" con (1) instalar proyecto nuevo (`runInit`, con selects de profile/overlay/target/layout/role servidos por `GET /api/launcher/options`), (2) unirse a proyecto existente (`runProjectsJoin`), (3) registrar/asignar roles (`runRolesRegister`/`runRolesAssign`), y un **explorador de carpetas** (`GET /api/launcher/browse`, lista subdirectorios, nunca lanza). Endpoints server-level `POST /api/launcher/{install,join}` (pre-proyecto) + project-scoped `POST /api/projects/:id/launcher/roles/{register,assign}` en el módulo `apps/cli/src/commands/app-onboarding.ts`. Tras install/join exitoso se refresca el selector de proyectos (`homeDirOverride`/`homeDir` = `state.homeDir`, mismo registry que lee el launcher).

...
Hay 10 adapter targets canónicos. Nueve tienen paquete dedicado y contrato común `generate<Target>Config(args) → Promise<Result<GeneratorResult, AdapterGeneratorError>>`: `opencode`, `claude-code`, `antigravity`, `visual-studio-2026`, `cursor`, `github-copilot`, `litellm`, `vscode` y `codex`; `copilot-vscode` es el único target sin paquete dedicado y usa el escritor de instrucciones compartido. `SupportLevel` de model-routing (`multi-mode`, `single-mode`, `fallback-only`) es un eje independiente de la existencia de un generador. Fuente: `Axiom/packages/adapters/README.md` y `apps/cli/src/commands/init.ts`.
...
- **`axiom mcp serve --kind axiom` — broker MCP unificado** (`INC-20260724-unified-axiom-mcp`): `--kind` acepta `axiom` además de `sdd`/`spec`/`memory`; lanza el `axiom-mcp-broker`. `axiom.config/mcp-manifest.yaml` declara un 3er id (`id: axiom`, `server: axiom-mcp-broker`, `projectBinding: required`); las entradas `sdd`/`spec` quedan sin cambios (retrocompat). El scaffold de proyectos adoptados (`DEFAULT_MCP_MANIFEST`) incluye el mismo 3er id. Detalle de tools en [06_Integraciones_y_Capacidades.md](06_Integraciones_y_Capacidades.md).

...
4. manifests/YAML que describen la configuración y la topología del proyecto (`axiom.yaml` como identidad/puntero local, y el único `axiom.config/topology.yaml` autoral en el repo `axiomRepo`);
...
- **`axiom repo add`**, **`axiom adapter add <target>`**, **`axiom provider add <id>`**, **`axiom role add <roleId> --path <path>`**: operaciones ADD-only sobre un workspace resuelto desde `axiomRepo` o desde un repo `code`, sin depender de que `projects.yml` exista. `repo add`/`role add` solo incorporan repos `code`; `repo attach` y los kinds `control|spec` ya no forman parte de la CLI. Estas dos mutaciones usan el preflight y coordinador estructural común: identidad/puntero, manifest autoral, bindings, `workspace.json` y registry solicitado se comprometen o recuperan como una unidad. `adapter add`/`provider add` actualizan el `WorkspaceStateV1` estricto mediante lock y writer atómico; los outputs derivados se regeneran post-commit y un fallo se informa como warning tipado sin convertir una estructura parcial en éxito. Los REMOVE siguen diferidos (NFR-AXM-015).
...
El reparto add/enable frente a repair/regenerate se deriva de
`WORKSPACE_STEP_CATALOG`, no de listas duplicadas. Cada step declara owner,
targets escribibles, inputs read-only, prerequisitos, preflight y clases de
output; `WORKSPACE_STEP_COMMAND_MATRIX` enlaza setup con `spec-base`, `skills`,
`adapters`, `rules`, `mcp-config` y `config-scaffold`, y deja exclusión explícita
para identidad/topología/estado/registry/code-intel/init-state.

Los comandos granulares prevalidan todos los targets antes del primer writer y
devuelven por step `created | updated | unchanged | skipped | failed` con
archivos, warnings y error tipado. `adapters` repara también sus process
surfaces sin invadir MCP; `rules` reemplaza solo la sección Axiom de `AGENTS.md`;
`config-scaffold` cubre las siete declaraciones/catálogos definidos; `mcp-config`
valida proyecto, target y path antes de escribir. Repair no añade adapters o
providers ni muta `workspace.json`/registry; esas altas siguen en los comandos
ADD explícitos.
...
### Contrato vigente del launcher local (R-13, ACC-070..ACC-072)

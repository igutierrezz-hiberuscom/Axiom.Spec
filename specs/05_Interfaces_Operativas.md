...
Hay 8 targets activos con paquete dedicado y contrato común `generate<Target>Config(args) → Promise<Result<GeneratorResult, AdapterGeneratorError>>`: `opencode`, `claude-code`, `antigravity`, `visual-studio-2026`, `cursor`, `github-copilot`, `vscode` y `codex`. `copilot-vscode` solo se acepta como alias legacy de `github-copilot`; LiteLLM fue retirado. `visual-studio-2026` delega en la instrucción común `.github/copilot-instructions.md` y no genera `.vs/AXIOM.md`. `SupportLevel` de model-routing (`multi-mode`, `single-mode`, `fallback-only`) es un eje independiente de la existencia de un generador. Fuente: `Axiom/packages/adapters/README.md` y `apps/cli/src/commands/init.ts`.
...
- `axiom mcp serve --kind axiom --project-root <path> [--home-dir <path>]` (`ACC-030`): lanza el único broker MCP ejecutable `axiom-mcp-broker` (`@axiom/mcp-server`) sobre stdio delimitado por saltos de línea. Expone juntos los dominios internos `sdd.*`, `spec.*`, `memory.*` y `axiom.*`, incluyendo lecturas y mutaciones confirmadas. Implementa MCP `2026-07-28` con `server/discover`, metadata namespaced, `resultType`, cache metadata y `-32022`; `initialize`, `notifications/initialized` y `ping` no forman parte del contrato. El proceso exige un binding project-scoped único y rechaza referencias cross-project. El comando se registra desde un fichero app-owned (`apps/cli/src/commands/mcp-serve.ts`).
...
6. `profile` (select);
7. `overlay` (select);
8. `adapters` (multi-select — cero o más adapters a instalar; opciones = los 10 `ADAPTER_TARGETS`; default pre-seleccionado `['opencode']`) — **supersede al step `target` `select` único** que describía la versión previa de este wizard (`INC-20260705-workspace-adapters-multiselect`);
...
`generateOpencodeMcpJson`/`generateClaudeCodeMcpJson` permanecen exportados y testeados, pero su call site de workspace fue retirado. `runWorkspaceSetup` escribe `.axiom/mcp.yml` en el repo de control y valida `projectId`, manifest, `enabled` y `targetRepo` antes de proyectar el schema MCP nativo de `opencode`, `claude-code`, `cursor`, `github-copilot` o `vscode`. La proyección es merge-preserving y atómica; elimina solo IDs gestionados por Axiom que ya no estén permitidos y conserva servidores custom. Visual Studio no recibe un schema MCP no verificado. `codex` y `antigravity` no reciben fichero ni recomendación global automática sin binding seguro. `.axiom/mcp.yml` se sigue escribiendo exactamente una vez por proyecto, y el provisioning de worktrees aplica el mismo filtro.
...
- **`axiom mcp serve --kind axiom` — broker MCP unificado** (`ACC-030`): `--kind` solo acepta `axiom`; `axiom.config/mcp-manifest.yaml`, `.axiom/mcp.yml`, el scaffold, `member-install` y las proyecciones nativas declaran una única entrada `axiom-mcp-broker`. Los prefijos de capability no son procesos ni aliases de servidor. Detalle de tools y protocolo en [06_Integraciones_y_Capacidades.md](06_Integraciones_y_Capacidades.md).

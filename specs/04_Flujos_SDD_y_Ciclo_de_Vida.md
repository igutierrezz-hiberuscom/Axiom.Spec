...
- **Efectos por transición declarados (P0)**: cada transición del grafo declara su mutación de YAML local y su llamada opcional al tracker (`{ localYaml, tracker }` + runner `transition-effects.ts`), sacando la lógica de lockstep de los wrappers de CLI al grafo; la variante `script/action` (git-services) queda declarada como extensión pendiente (ver [06_Integraciones_y_Capacidades.md](06_Integraciones_y_Capacidades.md)).

...
- **ARQUITECTO** (`axiom-role-planner`, en el **repo de spec**): lee spec + contexto técnico y emite **un plan por rol registrado** con repos destino y `allowedWriteScope`, usando el **contexto del spec-MCP** + code-intel (serena/codegraph) con **fallback físico** (Read/Grep) cuando el code-intel MCP no está disponible.

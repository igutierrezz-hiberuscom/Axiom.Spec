...
- La frontera documental está verificada: `Axiom.Spec/` es el repo canónico del workspace y `Axiom/axiom.spec/` es baseline product-owned materializable del runtime, conservada en su ubicación porque catálogos, adapters, readiness y artifact store la consumen. La autoridad topológica vigente es `Axiom.Spec/axiom.config/topology.yaml` (`schemaVersion: 2`, `axiomRepo` + `codeRepos` + `legacyRepos`); `Axiom/axiom.yaml` es identidad `code` y apunta a `../Axiom.Spec`. ADR-0032 documenta la frontera documental.

...
Puerta de entrada obligatoria al conocimiento técnico estable del producto Axiom, tal como existe en el código real de `Axiom/`. Baseline original: 2026-07-02; reconciliado el 2026-09-13 (R-15; ver "Fuentes auditadas y última validación" abajo). Cada documento enlazado aquí cita su fuente (código, README de package, docs oficiales o `Axiom.Spec/decisions/`) y distingue explícitamente entre estado verificado y planificación futura.
...
- Monorepo npm workspaces en `Axiom/`: `apps/cli` + **41 packages activos** bajo `packages/*` — **33 top-level** (`packages/adapters/` no tiene `package.json` propio, `@axiom/tui` fue físicamente retirado en ACC-081) + **8 adapters activos** (`opencode`, `claude-code`, `github-copilot`, `vscode`, `cursor`, `codex`, `antigravity`, `visual-studio-2026`). `@axiom/adapters-litellm` fue retirado; `copilot-vscode` no es un target público y solo se migra, si aparece persistido en `init.json`, durante `axiom configure` antes de instalar o despachar. Packages nuevos desde el baseline 2026-07-02 incluyen `launcher`, `mcp-server`, `mcp-tools`, `providers`, `technical-context`, `telemetry`, `tracker`, `tracker-ado`.
...
- La frontera documental está verificada: `Axiom.Spec/` es el repo canónico del workspace y `Axiom/axiom.spec/` es baseline product-owned materializable del runtime, conservada en su ubicación porque catálogos, adapters, readiness y artifact store la consumen. La autoridad topológica vigente es `Axiom.Spec/axiom.config/topology.yaml` (`schemaVersion: 2`, `axiomRepo` + `codeRepos` + `legacyRepos`); `Axiom/axiom.yaml` es identidad `code` y apunta a `../Axiom.Spec`. `Axiom/docs/**` es el manual único del runtime; `Axiom.Spec/specs/manuales/` es específico de esta instalación y no se distribuye. ADR-0032 documenta la frontera documental.
- El manual runtime se embebe como bundle TypeScript en `@axiom/document-bootstrap`, generado desde `Axiom/docs/**` antes del build, y se materializa en `docs/axiom/` mediante `workspace setup`, `workspace adopt` y `axiom upgrade`. `manifest.json` registra `sourceHash` y hashes SHA-256 por archivo; ediciones locales quedan `stale` y la versión nueva se separa bajo `.stale/`. El writer es atómico, project-root guarded, idempotente y no se ejecuta en `sync` ni `configure`.
- Las mutaciones estructurales de workspace (`setup`, `adopt`, `repo add`, `role add`) comparten preflight read-only, locks/replan y journal recuperable bajo `.axiom-state/<projectKey>/structural-transactions/<operationId>/`. Solo `ENOENT` significa ausencia; drift o I/O no observable falla closed. `axiom.yaml` se reconcilia dentro de `AXIOM:MANAGED`, los recursos estructurales terminan en commit/rollback/`recovery-required`, y adapters/reglas/MCP/catálogos se ejecutan después como outputs derivados con warnings tipados. Fuentes: `Axiom/apps/cli/src/commands/workspace-{structural-plan,structural-transaction,structural-types,axiom-yaml,derived-steps,step-catalog}.ts`.
...
2. **RESUELTA — cobertura documental de comandos, 2026-09-13**: las familias registradas por Commander tienen página en `Axiom/docs/cli/` y el test de cobertura deriva el conjunto desde la ayuda compilada. El manual completo se distribuye además en `docs/axiom/` por el writer único de `@axiom/document-bootstrap`.
...
- Reconciliación R-15 (2026-09-13, `ACC-088..ACC-095`): `Axiom/docs/**` quedó
	como manual único del runtime y sus familias CLI se cubren desde Commander.
	`@axiom/document-bootstrap` genera el bundle desde ese árbol antes del build
	y lo distribuye en `docs/axiom/` durante setup, adopción y upgrade explícito,
	con manifest/hash, preservación de ediciones locales como `stale` y salida
	nueva bajo `.stale/`; `sync` y `configure` quedan fuera. El cierre documental
	gobernado usa una declaración estructurada de documentos revisados y se
	integra en el runner común de CLI, launcher y MCP. El lote R-15 archivó sus
	seis incrementos; la suite focal del incremento 6, typecheck, build, doctor,
	readiness y diff check pasaron. La suite global clasificó `3742/3764` tests
	verdes en `346/358` archivos; sus 22 fallos quedaron fuera de R-15 por ser
	contratos/timeouts preexistentes de R-14/R-13, incluida la repetición de
	`workspace-step-reconciliation.test.ts`.
...
## Reconciliación R-13 (2026-09-08)

El control plane del launcher quedó verificado en el estado observado del runtime: loopback/sesión/origin, schemas y límites, containment de static/browse, SSE acotado con cierre explícito, `HttpLaunch` configurado y protegido contra SSRF/redirect/DNS rebinding, clipboard con ack/evidence, ausencia de transporte VS Code ficticio y grants server-side single-use ligados al snapshot. La implementación vive en `Axiom/`; el contrato estable se consolidó en `specs/02`, `05`, `06`, `07` y en los documentos de contexto de arquitectura, integraciones y operaciones. La evidencia operativa de R-13 incluye la matriz hermética ACC-070..ACC-076 y sus comprobaciones de no-mutación, junto con candidate freezes y receipts Core; la provenance de este pase es worktree-scoped/shared y no depende de un commit focal.